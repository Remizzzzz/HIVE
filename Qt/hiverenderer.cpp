    ...
    if (button) {
        ...
        if (!getInput()){//Si c'est la deuxième selection
            if (lastClicked!=nullptr){
                if (lastClicked->getInsectType()==insect){
            ...
            if (button->getInsectType()==insect){
...
void hiveRenderer::resizeEvent(QResizeEvent *event) {
    QMainWindow::resizeEvent(event);  // Conserver le comportement par défaut
}
/*
 * #include "hiverenderer.h"
#include "hexagonalbutton.h"
