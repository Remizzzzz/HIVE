...
public:
    ...
    int getFormerNeighbour(vec2i oldPosition, vec2i newPosition, Map &m) const; // getter pour détecter les anciens voisins à la nouvelle position
...
public:
    ...
    [[nodiscard]] std::string getPrintableValue(int idColor) const override {
        if(idColor == 0) {
            std::string color;
            std::stringstream s;
            if(getColor() == true ) color = "\033[34m";
            else color = "\033[31m";
...
public:
    Mosquitoe(bool col) : Insect(col, mosquitoe,"M"), Ant(col), Bee(col), Grasshopper(col), Beetle(col), Spider(col), Ladybug(col) {}
